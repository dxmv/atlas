package ast;

public class RpnPrinter implements Visitor<String>{
    @Override
    public String visitBinaryExpr(BinaryExpr expr) {
        return expr.left.accept(this) + " " +
                expr.right.accept(this) + " " + expr.operator.getValue();
    }

    @Override
    public String visitGroupingExpr(GroupingExpr expr) {
        return expr.expression.accept(this);
    }

    @Override
    public String visitLiteralExpr(LiteralExpr expr) {
        return expr.value.toString();
    }

    @Override
    public String visitUnaryExpr(UnaryExpr expr) {
        return expr.right.accept(this) + " " + expr.operator.getValue();
    }

    @Override
    public String visitVariableExpr(VariableExpr expr) {
        return "";
    }

    @Override
    public String visitAssignExpr(AssignExpr expr) {
        return "";
    }

    @Override
    public String visitPrintStmt(PrintStmt expr) {
        return "";
    }

    @Override
    public String visitExpressionStmt(ExpressionStmt expr) {
        return "";
    }

    @Override
    public String visitDeclareStmt(DeclareStmt expr) {
        return "";
    }

    @Override
    public String visitBlockStmt(BlockStmt expr) {
        return "";
    }

    @Override
    public String visitIfStmt(IfStmt expr) {
        return "";
    }

    @Override
    public String visitLogicalExpr(LogicalExpr expr) {
        return "";
    }

    @Override
    public String visitWhileStmt(WhileStmt expr) {
        return "";
    }

    @Override
    public String visitCallExpr(CallExpr expr) {
        return "";
    }

    @Override
    public String visitFunctionStmt(FunctionStmt expr) {
        return "";
    }

    @Override
    public String visitRetrunStmt(RetrunStmt expr) {
        return "";
    }

    public String print(Expr expression) {
        return expression.accept(this);
    }
}
